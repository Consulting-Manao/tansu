...
/// Build signed Git identity parameters.
///
/// Builds the message:
///   "Stellar Signed Message:\n" || member_address || git_pubkey || git_identity
/// and signs it directly with a deterministic Ed25519 keypair (fixed seed
/// `0x42`) so tests are reproducible.
fn signed_git_params(
    ...
    // Build the raw message: "Stellar Signed Message:\n" || member_address || pubkey || identity
    ...
    // Build the SSHSIG tosign payload (matches verify_git_signature):
...
fn add_member_with_git_identity() {
    ...
    // Verify the event includes the git identity
    let all_events = setup
        .env
        .events()
        .all()
    ...
    // Verify the stored member has git identity fields
    let info = setup.contract.get_member(&member);
    assert_eq!(
        info.git_identity,
...
fn add_member_git_identity_missing_pubkey() {
    ...
    // Pass identity but None pubkey → InvalidGitIdentity
    let error = setup
        .contract
