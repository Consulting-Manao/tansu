...
| `outcomes.json` | yes | Descriptions/XDR for the approved, rejected and cancelled outcomes. |
| `discussion.json` | no | Discussion thread rendered on the proposal page. |
| `summary.md` | no | Short Markdown summary of the discussion. |

`discussion.json` is either an array of posts or `{ "posts": [...] }`, where each post is:
...
```json
{
  ...
  "source": "https://optional-link-to-original"
}
```

`timestamp` accepts an ISO-8601 string or a unix timestamp (seconds or milliseconds); `source` is optional. The discussion directory defaults to the proposal's own (immutable) IPFS directory, so discussion content is fixed during voting.
...
- Proposal creation: **5 XLM** (`PROPOSAL_COLLATERAL`)
- Each badge vote: **2 XLM** (`VOTE_COLLATERAL`)
- Badge proposal creation (proposer): **7 XLM** total (5 XLM proposal + 2 XLM automatic abstain vote deposit)
- Token proposal creation (proposer): **5 XLM** only (voters lock the voting token, not XLM)
...
**Badge-based proposals:** Any authenticated Stellar address can vote with a minimum weight of 1 (the `Default` badge). Registered members with assigned badges can vote with higher weights up to their `get_max_weight` value. Each vote locks **2 XLM** collateral as the economic barrier to spam. See [Membership & Badges](./membership.mdx) for badge weights and the NQG-weighted project.

**Token-based proposals:** Any address holding the proposal's token can vote. Vote `weight` is in **whole token units** (the contract scales transfers by the token's `decimals()`). That amount is locked as collateral. Badge validation is skipped.
