...
`proposal_discussion.md` is the Markdown snapshot of the proposal's discussion, as produced by the proposals uploader alongside `proposal.md` and `outcomes.json`. It is rendered as a single Markdown document. The discussion directory defaults to the proposal's own (immutable) IPFS directory, so discussion content is fixed during voting.
...
To prevent spam and ensure serious proposal submissions, Tansu requires a proposal collateral deposit:
...
- Proposal collateral is **fully refunded** to the proposer when the proposal is executed
- Proposer gets collateral back even if rejected
- Exception: **Revoked proposals** forfeit proposal collateral (malicious/spam deterrent)
