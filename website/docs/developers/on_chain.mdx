...
Maintainers can also attach generic supply-chain evidence to a specific commit. Evidence is keyed by project, commit hash, and kind (`Sbom`, `Cve`, or `Attestation`). The contract stores only an off-chain content identifier; it does not upload artifacts, parse evidence contents, or verify attestations on-chain.
