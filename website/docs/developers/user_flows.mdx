...
flowchart TD
  ...
  subgraph Frontend
    ...
    D["Queries (TanStack Query, kept in IndexedDB)"]
    F["Writes (ProjectService, ProposalService, ...) via TxService"]
  end
  subgraph Blockchain
  ...
  A--"UI Interactions"-->C
  C--"Reads"-->D
  D--"RPC reads, account activity"-->K
  D--"README, commits"-->L
  D--"tansu.toml, proposals, profiles"-->M
  C--"Writes"-->F
  B--"Signs"-->F
  F--"Uploads"-->M
  F--"Transactions"-->K
  F--"Refetches what changed"-->D
  K--"Calls"-->J
```
...
Maintainers register projects with a registration deposit, name uniqueness check, and metadata storage.
...
sequenceDiagram
  ...
  D->>SC: Check Name Availability
  SC-->>D: Name Status
  U->>D: Submit Registration
  D->>IPFS: Upload tansu.toml
  IPFS-->>D: Return CID
  D->>W: Sign register() Transaction
  Note over W,SC: Takes the 5 XLM registration deposit
...
- **ProjectService.registerProject()**: Uploads the project's files and registers it, through the one write path (`TxService.sendTransaction`)
- **Name validation**: Real-time name availability checking via contract

#### Validation Steps

1. **Project Name**: 4-30 letters and digits, unique check via contract
2. **Maintainers**: Valid Stellar addresses (G...) and provider-specific maintainer aliases or handles
3. **Repository**: Valid supported git provider URL or public Radicle RID/URL
4. **Organization Info**: Required fields with URL format validation

#### Security Features

- **Registration deposit**: 5 XLM, not returned
- **Name uniqueness**: On-chain check prevents duplicate project names
...
- **Name conflicts**: Clear error messages when the project name is already taken
...
sequenceDiagram
    ...
    activate Contract
    Note over Contract: Deposit PROPOSAL_COLLATERAL (5 XLM)
    ...
    activate Contract
    Note over Contract: Validate commitment structure (3 G1 points)<br/>Check voter eligibility & weight
    ...
    activate Contract
    Note over Contract: Return proposal collateral to proposer
...
Contract badges (see [Membership & Badges](./membership.mdx)):

- **Developer**, **Triage**, **Community**, **Verified**, **Default**

Voting weight may also come from token balance (token-weighted proposals) or from the NQG contract (SCF Public Goods project).
...
Maintainers can update project commit hashes to track code versions on-chain. They can also attach generic off-chain evidence pointers to a specific commit for SBOMs, CVE scan reports, or attestations.
...
sequenceDiagram
  ...
  D->>R: Verify Commit Exists
  R-->>D: Commit Verification
  M->>SC: Submit commit()
  SC-->>D: Commit Updated
  D->>M: Show Success
```
...
- **Repository provider integration**: Verify commit hashes via hosted forge APIs or Radicle seed (GitHub, GitLab, Bitbucket, Codeberg, Gitea, Radicle)
- **Hash Format Validation**: 40 (SHA-1) or 64 (SHA-256) hexadecimal characters, enforced on-chain
- **Authorization Check**: Only project maintainers can update commits

### Historical Tracking

- **Event History**: The contract stores the latest hash; every update emits a `Commit` event, so the full history is on the ledger
- **Direct Reads**: The dApp reads current commit state from the contract without a backend indexer
- **Timestamp Accuracy**: Ledger timestamps provide precise commit timing

### Commit Evidence

- **Generic API**: Evidence uses `set_evidence` and `get_evidence`; there are no separate SBOM, CVE, or attestation-bundle endpoints. Maintainers' endorsements of a commit or artifact are a separate mechanism, see [Code Finality](./code_finality.mdx).
- **Scoped Storage**: Evidence is stored by `project_key`, `commit_hash`, and evidence kind.
- **Off-Chain Contents**: The contract stores only a CID and ledger timestamp. Artifact upload, parsing, and verification happen off-chain by fetching the CID from IPFS.
