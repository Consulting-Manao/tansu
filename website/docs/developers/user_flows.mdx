...
Maintainers register projects with a collateral deposit, name uniqueness check, and metadata storage.
...
sequenceDiagram
  ...
  Note over W,SC: Locks 5 XLM collateral
...
- **Collateral deposit**: 5 XLM locked on registration
...
- **Generic API**: Evidence uses `set_evidence` and `get_evidence`; there are no separate SBOM, CVE, or attestation endpoints.
